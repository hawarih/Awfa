...
Planned: Future activities.""", selection=[('overdue', 'Overdue'), ('today', 'Today'), ('planned', 'Planned')], readonly=True)
    ...
    department_id = fields.Many2one(comodel_name='hr.department', string='Department', help="""Department Of The Employee""", related='employee_id.department_id', readonly=True, store=True)
    employee_id = fields.Many2one(comodel_name='hr.employee', string='Employee', readonly=True, store=True)
    end_date = fields.Datetime(string='End Date', help="""End date of the Request""", copy=True, required=True, store=True, )
    has_message = fields.Boolean(string='Has Message', readonly=True)
    is_hr_manger = fields.Boolean(string='Is Hr Manger', readonly=True)
    is_refuse = fields.Boolean(string='Is Refuse', readonly=True)
    is_validate = fields.Boolean(string='Is Validate', readonly=True)
    ...
    note = fields.Html(string='Note', copy=True, store=True)
    number_of_hours = fields.Float(string='Number Of Hours', help="""Number Of OverTime Hours""", readonly=True, )
    number_of_hours_in_portal = fields.Char(string='Number Of Hours In Portal', help="""Number Of OverTime Hours""", readonly=True, )
    number_of_minutes = fields.Float(string='Number Of Minutes', help="""Number Of OverTime In Minutes""", readonly=True, )
    seq_name = fields.Char(string='Serial No', required=True, readonly=True, index=True, store=True)
    start_date = fields.Datetime(string='Start Date', help="""Start date of the Request""", copy=True, required=True, store=True, )
    state = fields.Selection(string='Status', selection=[('draft', 'Draft'), ('waiting_direct_manager_approval', 'Waiting Direct Manager Approval'), 
                                                         ('waiting_hr_responsible_approval', 'Waiting Hr Responsible Approval'), ('waiting_hr_manager_approval', 'Waiting HR Manager Approval'), 
                                                         ('waiting_head_sector_approval', 'Waiting Head Sector Approval'), ('approve', 'Approved'), ('refuse', 'Refuse')], default='draft')
    ...
    def hr_responsible_approve(self):
        pass
