...
Planned: Future activities.""", selection=[('overdue', 'Overdue'), ('today', 'Today'), ('planned', 'Planned')], readonly=True)
    ...
    assign_vehicle = fields.Many2one(comodel_name='fleet.vehicle', string='Assign Vehicle', copy=True, store=True, )
    current_location_id = fields.Many2one(comodel_name='stock.location', string='Current Location', readonly=True)
    ...
    name = fields.Char(string='GPS Name', copy=True, readonly=True, store=True, )
    related_product = fields.Many2one(comodel_name='product.template', string='Related Product', copy=True, store=True, )
    scrap_ids = fields.One2many(comodel_name='stock.scrap', inverse_name='gps_id', string='Scrap', store=True, )
