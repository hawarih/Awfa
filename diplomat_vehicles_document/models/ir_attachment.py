...
Planned: Future activities.""", selection=[('overdue', 'Overdue'), ('today', 'Today'), ('planned', 'Planned')], readonly=True)
    ...
    doc_type_id = fields.Many2one(comodel_name='doc.type', string='Document Type', copy=True, store=True)
    end_date = fields.Date(string='End Date', copy=True, store=True)
    fleet_doc = fields.Boolean(string='fleet', copy=True, readonly=True, store=True)
    ...
    notify = fields.Boolean(string='Notification', copy=True, store=True)
    num_days = fields.Integer(string='Number Of days before End', copy=True, store=True)
    related_user_ids = fields.Many2many(comodel_name='res.users', string='Notification Users ', copy=True, relation='ir_attachment_res_users_rel', column1='ir_attachment_id', column2='res_users_id', store=True)
