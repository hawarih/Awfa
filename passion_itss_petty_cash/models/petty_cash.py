...
class PettyCash(models.Model):
    ...
    balance2 = fields.Monetary(string='Balance value', copy=True, readonly=True, store=True)
    ...
    employee_id = fields.Many2one(comodel_name='hr.employee', string='Employee To assign', copy=True, required=True, ondelete='restrict', store=True)
    ...
    line_ids = fields.One2many(comodel_name='petty.cash.line', inverse_name='petty_id', string='Lines', store=True)
    ...
    paid = fields.Boolean(string='Is Paid', readonly=True)
    pay_journal_id = fields.Many2one(comodel_name='account.journal', string='Payment Journal', copy=True, store=True)
    payment_count = fields.Integer(string='# of Payments', readonly=True)
    payment_ids = fields.Many2many(comodel_name='account.payment', string='Payments', relation='account_payment_petty_cash_rel', column1='petty_cash_id', column2='account_payment_id', store=True)
    reference = fields.Char(string='Reference', copy=True, store=True)
    remain_amount = fields.Monetary(string='Remaining Amount', copy=True, store=True)
    state = fields.Selection(string='State', selection=[('draft', 'Draft'), ('approved', 'Approved'), ('paid', 'Paid'), 
                                                        ('reconciled', 'Reconciled'), ('adjust', 'Adjusted')], 
                                                default='draft',store=True)
    type_id = fields.Many2one(comodel_name='petty.cash.type', string='Petty Cash Type', copy=True, store=True)
