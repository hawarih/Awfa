...
Planned: Future activities.""", selection=[('overdue', 'Overdue'), ('today', 'Today'), ('planned', 'Planned')], readonly=True)
    ...
    amount_remaining = fields.Float(string='Amount Remaining', readonly=True)
    amount_total = fields.Float(string='Amount Total', readonly=True)
    date = fields.Date(string='Start Date', copy=True, store=True)
    department_id = fields.Many2one(comodel_name='hr.department', string='Department', related='employee_id.department_id', readonly=True)
    duration = fields.Integer(string='Duration', copy=True, store=True)
    ...
    notes = fields.Html(string='Notes', copy=True, store=True)
    rate = fields.Float(string='Rate', copy=True, store=True, )
    request_date = fields.Datetime(string='Request Date', related='create_date', readonly=True)
    settlement_line_ids = fields.One2many(comodel_name='settlement.line', inverse_name='allowance_deduction_id', string='Settlement Line', store=True)
    state = fields.Selection(string='Status', selection=[('draft', 'Draft'), ('waiting', 'Waiting Hr Approve'), ('done', 'Done'), ('cancel', 'Cancelled')], 
